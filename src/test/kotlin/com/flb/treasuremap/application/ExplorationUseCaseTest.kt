...
class ExplorationUseCaseTest {
    ...
    fun an_explorer_should_go_ahead_horizontally() {
        ...
        val treasureMap = TreasureMap(Explorer(Position(Pair(0, 0), orientation)))
        ...
        assertThat(treasureMap).isEqualTo(
            TreasureMap(Explorer(Position(Pair(1, 0), orientation)))
        )
    }

    @Test
    fun an_explorer_should_go_ahead_vertically() {
        // given
        ...
        // then
        assertThat(treasureMap).isEqualTo(
            TreasureMap(Explorer(Position(Pair(0, 1), orientation)))
        )
    }
}
